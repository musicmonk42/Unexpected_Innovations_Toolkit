...
if os.name == "nt":  # Windows
    try:
        ...
        # Fallback to standard RotatingFileHandler if concurrent_log_handler is not installed
        logging.warning("concurrent-log-handler not found. Falling back to standard RotatingFileHandler on Windows. "
                        "Consider installing it for better log rotation on Windows (pip install concurrent-log-handler).")
...
class JSONFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Flatten extras into the log data (assuming no key conflicts; nest if needed)
...
class HumanReadableFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        # Assuming redact_secrets is defined elsewhere or not strictly necessary for this example
        # For demonstration, we'll just use record.getMessage() directly
...
class ErrorTracker:
    ...
    def __init__(self, log_file: str = 'errors.log', level: int = logging.INFO,
        ...
        # --- FIX: Add stateful tracking attributes ---
        self._errors = deque()
        self.notifier = None
    ...
    # --- FIX: Add missing methods expected by the application ---
    ...
    def summarize_and_notify(self, operation: str, verbose: bool = False, severity: str = "ERROR") -> None:
        ...
        if self.has_errors():
            ...
            if self.notifier:
                ...
                # Only send email for more critical severities
